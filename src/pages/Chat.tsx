  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || error.error || 'Tuffy AI is unavailable');
      }
      ...
      setMessages(prev => [...prev, {
        ...
        content: error instanceof Error
          ? `${error.message}. Start the API with \`npm run server\` and configure OPENAI_API_KEY to enable live Tuffy AI responses.`
          : 'Tuffy AI is currently unavailable.',
        timestamp: new Date(),
      }]);
    } finally {
      setIsLoading(false);
